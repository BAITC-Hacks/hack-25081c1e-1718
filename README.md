...
Статус: ТЗ, план на 300 минут, адаптивное ядро и OpenAI-советник реализованы. Оба реальных API-вызова прошли. Автономный seed 42: net +21 710, но лишь 3 из 10 seed положительны; устойчивость не достигнута. См. [измерения](docs/BASELINE.md) и [запуск / передача Азиму](docs/RUNBOOK.md). Модуль данных и веб — следующая часть Азима.
...
Ядро выбирает динамические гипотезы, проводит разведку и повторные пилоты, затем составляет непересекающийся план с контролем ресурсов. candidate_model.py Азима подключается автоматически по контракту; пока используется предварительный подбор по ценам и сегментам. Не используется чтение скрытых эффектов. Публичный mock служит для проверки механики; его результат не равен результату на судействе.

Вычислительное ядро работает без сети и API-ключа. LLM-адаптер читает OPENAI_API_KEY из окружения и имеет fallback. Для онлайн-отчёта: `python -X utf8 scripts/run_agent.py --ask-key` (скрытый ввод только в память процесса). Не помещать ключи в Git, браузер и отчёты. Официальный CSV генерируем с ARPU_OFFLINE=1 для повторяемости.
